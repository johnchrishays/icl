...
def plot_mixture_contours(means, vars, resolution=100, range_factor=3):
    fig, ax = plt.subplots(figsize=(10, 8))
    
    x_min = means[:, 0].min() - range_factor * np.sqrt(vars[:, 0].max())
    x_max = means[:, 0].max() + range_factor * np.sqrt(vars[:, 0].max())
    y_min = means[:, 1].min() - range_factor * np.sqrt(vars[:, 1].max())
    y_max = means[:, 1].max() + range_factor * np.sqrt(vars[:, 1].max())
    ...
    for i in range(len(means)):
        ...
        Z += np.exp(-0.5 * mahalanobis) / np.sqrt(np.linalg.det(var))
    Z = Z.reshape(X.shape) / len(means)
    
    # Apply log scaling for potentially better contrast
    epsilon = 1 
    log_Z = np.log(Z + epsilon)
    
    contour = ax.contourf(X, Y, log_Z, levels=20, cmap='magma')
    ax.scatter(means[:, 0], means[:, 1], color='red', s=50, marker='x')
    ax.set_xlabel('Dimension 1')
    ax.set_ylabel('Dimension 2')
    ax.set_title('Mixture of Gaussians Contour Plot')
